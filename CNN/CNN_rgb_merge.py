...
logger.info(
'MINIBATCH_SIZE: {}, STEPS: {}, PIXEL: {}, COLOR: {}, CONV: {}, DROP_KEEP: {}, EPOCH: {}, LEARNING: {}'.format(MINIBATCH_SIZE, 
...
conv1 = conv_layer(x, shape=[CONV, CONV, COLOR, CONV_DEEP])
conv1_pool = max_pool_2x2(conv1)

conv2 = conv_layer(conv1_pool, shape=[CONV, CONV, CONV_DEEP, CONV_DEEP * 2])
conv2_pool = max_pool_2x2(conv2)

new_size = int(PIXEL/4*PIXEL/4*CONV_DEEP * 2)
...
with tf.Session() as sess:
    ...
    for i in range(STEPS):
        X_batch, y_batch, file_name = train_img.get_batch(MINIBATCH_SIZE)
        ...
        if i % STEPS/100 == 0:
            ...
            while val_counter <  val_number//val_batch:
                ...
                val_sum.append(val_accuracy)
                val_entro_sum.append(cross_entro_val)


            vX_batch, vy_batch, file_name = val_img.get_batch(val_batch) 
            val_accuracy = sess.run(accuracy, feed_dict={x: vX_batch, y_: vy_batch, keep_prob: 1.0}) 
            cross_entro_val = sess.run(cross_entropy, feed_dict={x: vX_batch, y_: vy_batch, keep_prob: 1.0})
            val_sum.append(val_accuracy *(val_number%val_batch)/val_batch)
            val_entro_sum.append(cross_entro_val*(val_number%val_batch)/val_batch)


            logger.info("step {}, validation accuracy {}".format(i, (np.sum(val_sum) / val_number) * val_batch))
            logger.info("step {}, validation cross entropy {}".format(i, (np.sum(val_entro_sum) / val_number) * val_batch))

            logger.info("step {}, train accuracy {}".format(i, np.mean(train_accuracy)))
            logger.info("step {}, train cross entropy {}".format(i, np.mean(cross_entro)))
...
#                cross_entro_sum.append(cross_entro)cd
#
...
#
    ...
    test_sum.append(test_accuracy *(test_number%test_batch)/test_batch)
